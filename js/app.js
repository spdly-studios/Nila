...
dateLabel.textContent = new Intl.DateTimeFormat('en-US', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }).format(new Date()).toUpperCase();
const wait = ms => new Promise(resolve => setTimeout(resolve, ms));
const escapeHtml = value => String(value ?? '').replace(/[&<>"']/g, character => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[character]));
const getRelayUrl = () => localStorage.getItem(RELAY_URL_STORAGE)?.trim().replace(/\/$/, '') || window.location.origin;
...
function render() { rows.innerHTML = students.map((student, index) => `<tr><td>${escapeHtml(student.name)}</td><td>${escapeHtml(student.roll)}</td><td><button class="status ${student.present ? 'present' : 'absent'}" data-index="${index}" aria-pressed="${student.present}">${student.present ? 'Present' : 'Absent'}</button></td></tr>`).join(''); updateCounts(); }
render();
