...
const server = http.createServer(async (request, response) => {
    ...
    if (request.method === 'POST' && requestUrl.pathname === '/api/attendance') {
        ...
        try { const record = JSON.parse(body || '{}'); if (!/^\d{4}-\d{2}-\d{2}$/.test(record.date) || !/^[0-9]+-[A-Z]$/.test(record.className || '') || !Array.isArray(record.students) || !record.students.length || record.students.some(student => typeof student?.name !== 'string' || typeof student?.roll !== 'string' || typeof student?.present !== 'boolean')) return sendJson(response, 400, { error: 'date, className, and valid students are required' }); const existingIndex = attendanceStore.findIndex(saved => saved.date === record.date && saved.className === record.className); const existing = existingIndex >= 0 ? attendanceStore[existingIndex] : null; record.id = existing?.id || record.id || crypto.randomUUID(); record.savedAt = new Date().toISOString(); if (existingIndex >= 0) attendanceStore[existingIndex] = record; else attendanceStore.push(record); persistAttendance(); appendLog(existing ? 'attendance.updated' : 'attendance.saved', { id: record.id, date: record.date, className: record.className, studentCount: record.students.length }); return sendJson(response, existing ? 200 : 201, record); }
