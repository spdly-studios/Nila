...
function ensureAnalysis(record, analysis) { const speech = parentSpeech(record.transcript), fallback = fallbackAnalysis(record); return { ...(analysis || fallback), summary: analysis?.summary || (speech ? speech.slice(0, 500) : (fallback.mainReason || 'Parent summary not available.')), mainReason: analysis?.mainReason && analysis.mainReason !== 'No reason provided' ? analysis.mainReason : fallback.mainReason, tags: Array.isArray(analysis?.tags) && analysis.tags.length ? analysis.tags : fallback.tags }; }
