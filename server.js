...
function parentSpeech(transcript) { const lines = String(transcript || '').split(/\r?\n/).filter(line => /^(parent|customer|caller|user)\s*:/i.test(line)); return lines.length ? lines.join('\n') : String(transcript || ''); }
